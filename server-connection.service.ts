...
export class ServerConnectionService {
  ...
  postFile<T>(url: string, ...params: any[]): Promise<T> {
    ...
    return new Promise<T>((resolve, reject) => {
      ...
      element.onchange = (event: any) => {
        ...
        this.http.post(environment.hostUrl + url, formData, {
          headers: headers,
  ...
  post<T>(url: string, data: any): Promise<T> {
    var dateParser = function (key, value) {
      if (typeof value === 'string') {
        var date = Date.parse(value);
        if (date && (value.indexOf('/') >= 0 || value.indexOf('-') >= 0)) {
          //console.log("date not NaN: ", date, value);
          return new Date(date);
        }
      }
    ...
    return new Promise<T>((resolve, reject) => {
      this.http.post(environment.hostUrl + url, data, { withCredentials: true }).subscribe((response: any) => {
        try {
          var json = JSON.stringify(response);
          var result = JSON.parse(json, dateParser);
          result = this.deserializeReferences(result, {}, []);
  ...
  fixDateTimeProperties<T>(data: any, oneTimeList: Array<T>) {
    if (data == null || data == undefined || oneTimeList.Any(x => x == data))
      return;
    oneTimeList.Add(data);
    let properties = Object.getOwnPropertyNames(data);
    //alert("properties: " + properties);
    properties.ForEach(x => {
      let pData = data[x];
      if (pData == null || pData == undefined || oneTimeList.Any(x => x == pData))
        return;
